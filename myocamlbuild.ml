(* OASIS_START *)
(* DO NOT EDIT (digest: bd81ad09e8d5a020a2a0cac630fa1200) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml = [("kmb/kmb", ["kmb"])];
     lib_c = [];
     flags = [];
     }
  ;;
...
let () =
  dispatch (fun hook -> 
    ...
    (match hook with
      | Before_rules ->
        rule "peg_parser.ml"
          ~deps:["kmb/peg.peg"; "kmb/kombain_init.byte"]
          ~prod:"kmb/peg_parser.ml"
          (fun _ _ ->
            Cmd (S [Px"kmb/kombain_init.byte"; A"kmb/peg.peg";
                    A"kmb/peg_parser.ml"])
          );
      
        flag_and_dep ["ocaml"; "compile"; "byte"; "file:kombain.ml"] &
          S [P"kmb/peg_parser.cmo"];
        
        rule "generate parser"
          ~deps:["kmb/kombain.byte"; "%.peg"]
          ~prod:"%_parser.ml"
          (fun env _ ->
            let peg = env "%.peg"
