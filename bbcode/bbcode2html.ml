  ...
  List.fold_right (fun t acc ->
    match t with
      ...
      | Image data ->
        ...
        in
          img ~a ~src:data.src ~alt:(if data.alt = "" then "[IMG]" else data.alt)
            () :: acc
      | Break -> br () :: acc
      | Size (value, is) ->
  ...
  List.fold_right (fun t acc ->
    match t with
      ...
      | SimpleTag (key, content) -> (
        match key with
          | "google" ->
            (a ~a:[a_href ("http://www.google.com/search?q=" ^ content)]
               [pcdata content]) :: acc
          | "wikipedia" ->
            (a ~a:[a_href ("http://www.wikipedia.org/wiki/" ^ content)]
               [pcdata content]) :: acc
          | "youtube" ->
            (object_
               ~params:[param ~a:[a_name "movie";
                                  a_text_value ("http://www.youtube.com/v/" ^ content)]
                           ()]
               ~a:[a_width 425; a_height 366]
               [embed ~a:[a_src ("http://www.youtube.com/v/" ^ content);
      ...
      | Url (url, text) ->
        let url =
          if (String.length url > 7 && String.sub url 0 7 = "http://") ||
            ...
            "http://" ^ url in
          (a ~a:[a_href url] [pcdata text]) :: acc
      | Flash ((w, h), url) ->
        (object_~params:[param ~a:[a_name "movie"; a_text_value url] ()]
           ~a:[a_data url; a_width w; a_height h;
               a_mime_type "application/x-shockwave-flash"] []) :: acc
      | other -> (make_phrasing [other] :> Html5_types.phrasing elt list) @ acc
  ) is []
...
let make_index style data =
  ...
  let make_link is =
    a ~a:[a_href ("#" ^ make_heading_anchor is)]
